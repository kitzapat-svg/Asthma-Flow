...
### 2026-07-01 - ปรับปรุงโครงสร้างระบบพิมพ์บัตรประจำตัวผู้ป่วยให้เป็นมาตรฐานเดียวกันและแก้ไขปัญหาฟอนต์ต่างดาว

เป้าหมาย:
- ยุบรวมดีไซน์และฟังก์ชันการพิมพ์บัตรของหน้าบัตรเดี่ยว (Patient Detail) และพิมพ์บัตรแบบกลุ่ม (Print Cards) ให้ดึงจากโมดูลเดียวกัน (`components/AlertCard.tsx`)
- แก้ไขปัญหาภาษาต่างดาว (gibberish/character encoding) ที่เกิดขึ้นบนหน้า Print Cards หลังจากใช้งาน PowerShell set-content

ทำเสร็จ:
- ดึงฟังก์ชัน `AlertCardInner`, `ThemeConfig`, `CARD_THEMES`, และ `sarabunFont` ออกมาสร้างเป็นคอมโพเนนต์ส่วนกลางใน `components/AlertCard.tsx`
- อัปเดต `app/staff/print-cards/page.tsx` และ `app/staff/patient/[hn]/page.tsx` ให้เรียกใช้งานคอมโพเนนต์ส่วนกลางนี้ร่วมกัน
- แก้ไขรหัสตัวอักษรของไฟล์ `app/staff/print-cards/page.tsx` ที่ได้รับความเสียหายจาก PowerShell encoding โดยการทำ Git restore และบันทึกทับด้วย UTF-8 ที่สมบูรณ์
- ปรับสไตล์ `@media print` ในหน้ารายละเอียดผู้ป่วยเดี่ยวเพื่อให้ตัวการ์ดจัดกึ่งกลางบนหน้ากระดาษ A4 เสมือนการพิมพ์จากระบบพิมพ์แบบกลุ่ม

Security/Privacy:
- การเปลี่ยนแปลงไม่มีผลกระทบต่อขอบเขตความปลอดภัย การแสดงรหัสผู้ป่วยและ QR Code ยังอิงตาม Public Token และ RLS/Access Control ใน API ปกติ

ปัญหา/ข้อสังเกต:
- ปัญหาภาษาต่างดาวเกิดจากการใช้คำสั่ง Get-Content ใน PowerShell 5.1 ซึ่งไม่บังคับ UTF-8 เป็นค่าเริ่มต้น จึงทำให้อ่านรหัสภาษาไทยเพี้ยนและบันทึกทับไป ต่อไปให้เลี่ยงการแก้ไขไฟล์ภาษาไทยผ่าน pipeline script หรือ utility command ที่ไม่บังคับ encoding

Next steps:
- นำระบบการจัดพิมพ์ไปใช้จริงในคลินิก และรวบรวมฟีดแบ็กจากหน้างาน

Verification:
- ดำเนินการทดสอบด้วย Next.js build ผ่านแบบไร้ข้อผิดพลาด (Exit code: 0)
- รัน Browser subagent ตรวจสอบหน้า `http://localhost:3000/staff/print-cards` และหน้ารายละเอียดผู้ป่วยเดี่ยว พบการแสดงผลภาษาไทยอย่างสมบูรณ์แบบ (ไม่มีภาษาต่างดาว) และ QR Code ชี้ลิงก์สุขภาพถูกต้องตาม HN
- ยืนยันภาพ Screenshot ของหน้าแสดงผลมีความสวยงามและจัดหน้ากระดาษตรงตามความต้องการ

### 2026-07-01 - จัดเรียงข้อมูลประวัติการมาตรวจตาม HN จากน้อยไปมาก

เป้าหมาย:
- จัดเรียงข้อมูลในตาราง "สรุปการมาตรวจ" (Summary of visits) ในหน้า Data Management ตาม HN จากน้อยไปมากตามคำขอของผู้ใช้

ทำเสร็จ:
- ปรับปรุงการเรียงลำดับใน `attendanceData` (`app/staff/data-management/page.tsx`) ให้เรียงลำดับตาม HN แบบตัวเลข (ascending) และเรียงตามวันนัดหมาย/วันตรวจจากล่าสุดลงไป (descending) สำหรับ HN เดียวกัน

Security/Privacy:
- ไม่มีผลกระทบต่อความปลอดภัยหรือความเป็นส่วนตัวของข้อมูล เป็นเพียงการเปลี่ยนลำดับการจัดเรียงบน UI หน้า Admin เท่านั้น

ปัญหา/ข้อสังเกต:
- ไม่มีปัญหา การทำงานราบรื่นดี

Next steps:
- ติดตามผลการใช้งานจากคลินิก

Verification:
- ใช้ Browser subagent เปิดหน้า `http://localhost:3000/staff/data-management` และตรวจสอบลำดับของ HN ในตาราง สรุปการมาตรวจ พบว่าเรียงลำดับจากน้อยไปมากถูกต้อง (เช่น 0001171, 0006485, 0039935, ...)
