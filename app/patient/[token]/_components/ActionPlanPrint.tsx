...
export function ActionPlanPrint({ patient, visit }: ActionPlanPrintProps) {
    ...
    return (
        <div className="hidden print:block w-[210mm] h-[297mm] bg-white text-black p-8 font-sans relative">
            {/* Header */}
            <div className="flex justify-between items-start border-b-2 border-black pb-4 mb-6">
                <div>
                    <h1 className="text-2xl font-black uppercase text-[#D97736]">Asthma Action Plan</h1>
                    <p className="text-sm font-bold text-gray-500">แผนการดููแลรักษาโรคหืดเบื้องต้น</p>
                    <div className="mt-4 space-y-1">
                        <p className="text-lg">ชื่อ-นามสกุล: <span className="font-bold border-b border-dotted border-black px-2">{patient.prefix}{patient.first_name} {patient.last_name}</span></p>
                        <p>HN: <span className="font-mono font-bold">{patient.hn}</span> อายุ: <span className="font-bold">{new Date().getFullYear() - new Date(patient.dob).getFullYear()}</span> ปี</p>
                    </div>
                </div>
                <div className="text-right flex flex-col items-end">
                    <div className="border p-2 mb-2">
                        <QRCodeSVG value={`https://asthsawan.vercel.app/patient/${patient.public_token}`} size={64} />
                    </div>
